    ...
    <div className={styles.step}>
      ...
      <div className={styles.form}>
        ...
        <ul className={styles.termsList}>
          {TERMS.map((term) => (
            <li key={term.id} className={styles.termRow} onClick={() => toggle(term.id)}>
              <span className={styles.termLabel}>
                {term.label}
                {term.required && <span className={styles.required}>(필수)</span>}
                {!term.required && <span style={{ color: 'var(--ink-3)', fontSize: 12, marginLeft: 4 }}>(선택)</span>}
              </span>
